...
- Le fichier `tests.ipynb` est un Jupyter notebook qui contient les résultats des fonctions implémentées ainsi que quelques tests sur les données.

**Script Python**

1. Partie 1. Gale-Shapley

- `gale_shapley.py` contient les algorithmes de Gale-Shapley ainsi que la fonction qui permet de trouver les paires instables.

- `matrice_pref.py` contient les fonctions qui manipulent les matrices. Elles permettent
...
3. Partie 3. PL(NE)
   Le code dans les fichiers suivants est adapté uniquement pour les données données par les fichiers `PrefEtu.txt` et `PrefSpe.txt`.
...
Chaque fichier Gurobi génère un fichier `.lp` avec le problème linéaire formulé.

**Dossiers**

- Le dossier `plne` contient tous les fichiers `.lp` générés.
